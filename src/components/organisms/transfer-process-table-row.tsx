...
export default function TransferProcessTableRow({
  transferProcess,
  managementUrl,
  connectorEndpoint,
  participantId,
}: AssetDetailsProps) {
  const cleanedTransferProcess =
  ...
  const [transferProcessDialogIsOpen, setTransferProcessDialogIsOpen] =
    useState(false);
  ...
  useEffect(() => {
    ...
    edcClient.management.contractAgreements
      .getNegotiation(transferProcess.contractId)
      .then((negotiation) => {
    ...
    edcClient.management.contractAgreements
      .get(transferProcess.contractId)
  ...
  useEffect(() => {
    ...
    edcClient.management.assets
      .get(transferProcess.assetId)
      .then(setAsset)
      .catch(() => setAsset({} as Asset));
  }, [edcClient, transferProcess.assetId]);

  const jsonLdObject = useTransferProcessJsonLd(
    transferProcess,
    contractNegotiation,
  );
  ...
  return (
    <>
      ...
      <Table.Row className="transfer-process-row">
        ...
        <Table.Cell>
          <Tooltip
            title={formatDateTime(stateTimestamp, { showDayOfWeek: true })}
          >
        ...
        <Table.Cell>{contractNegotiation.counterPartyId}</Table.Cell>
        <Table.Cell>{contractNegotiation.counterPartyAddress}</Table.Cell>
        <Table.Cell>
          <p className="text-xs italic mb-1">
            {`${contractAgreement.providerId} → ${contractAgreement.consumerId}`}
          </p>
          <p className="font-semibold">{contractAgreement.id}</p>
        </Table.Cell>
        <Table.Cell>
          <span
