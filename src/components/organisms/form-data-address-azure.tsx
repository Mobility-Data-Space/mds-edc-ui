...
export function FormDataAddressAzure({
  ...
  translator,
  isDestination = false,
}: FormDataAddressAzureProps): React.ReactElement {
  ...
  return (
    <>
      <div>
        <Checkbox
          data-testid="multiple-blobs-checkbox"
          label={translator("assets.new.fieldMultipleBlobs")}
          value={multipleBlobs}
          onChange={(event) => setMultipleBlobs(event.target.checked)}
        />
      </div>
      <div>
        <label
