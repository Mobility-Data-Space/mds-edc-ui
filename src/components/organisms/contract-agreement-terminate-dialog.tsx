...
import {TERMINATION_DETAILED_REASON_MAX_LENGTH, TERMINATION_REASON_BY_USER} from "@/constants/contract-agreement.ts";
...
function calculateDetailedReasonValidationError(detailedReason: string, translator: (key: string) => string) {
  if (! detailedReason) {
...
export default function ContractAgreementTerminateDialog({ contractAgreement, open, onClose, onSuccess = () => {}, translator }: ContractAgreementTerminateDialogProps) {
  ...
  const detailedReasonError = calculateDetailedReasonValidationError(formData.detailedReason, translator);
        ...
        <div className="flex flex-col gap-y-5">
          ...
          <div className="flex flex-col">
            <Input
              ...
              placeholder={translator("contractAgreements.[id].detailedReasonPlaceholder")}
              multiline
              required
              rows={6}
              ...
              onChange={(event) => setFormData({ ...formData, detailedReason: event.target.value })}
            />
          ...
          <Checkbox
            ...
            onChange={(event) => setFormData({ ...formData, confirmTermination: event.target.checked })}
          />
        </div>
      </DialogContent>
      <DialogActions>
